...
export default function SymptomCheckerPage() {
  ...
  /**
   * A robust helper function to safely parse the text from any AI response format.
   * @param result The JSON object returned from the fetch call.
   * @returns The displayable text string.
   */
  type AIResponse = {
    output?: {
      output?: string;
  ...
  const parseAIResponse = (result: AIResponse): string => {
    ...
    if (typeof result.output?.output === 'string') {
    ...
    try {
      ...
      if (mode === 'symptom') {
        response = await fetch(
          `${API_BASE_URL}/api/patient/chat/invoke`,
          {
            method: 'POST',
        ...
        response = await fetch(
          `${API_BASE_URL}/api/appointment/chat`,
          {
            method: 'POST',
      ...
      if (result.output?.action === 'collect_crisis_details') {
        setCrisisMessage(result.output.empathetic_message);
      ...
      <div className="w-full md:w-1/4 bg-blue-900 p-4 flex flex-col text-white">
        ...
        <div className="flex gap-2 mb-4">
          <button
            ...
            className={`flex-1 px-3 py-2 rounded ${mode === 'symptom'
          ...
          <button
            ...
            className={`flex-1 px-3 py-2 rounded ${mode === 'appointment'
        ...
        <div className="flex-1 overflow-y-auto mt-4">
          <h2 className="text-sm font-semibold text-blue-200 mb-2">
            Recent Chats
          </h2>
          ...
          <div className="w-full max-w-4xl flex-1 mb-4 bg-white rounded-lg shadow-sm border border-gray-200 flex flex-col">
            ...
            >
              {messages.map((msg, index) => (
                ...
                >
                  <div
                    className={`max-w-3xl rounded-lg px-4 py-3 ${msg.sender === 'user'
                      ? 'bg-blue-600 text-white'
